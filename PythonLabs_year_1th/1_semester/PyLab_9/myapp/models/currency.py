...
class Currency:
    ...
    def id(self, id_: str):
        if isinstance(id_, str) and id_ != "" and re.match(r'[a-zA-Z]', id_[0]) is not None:
            ...
            raise ValueError("id must be a nonempty string start with a letter")
    ...
    def num_code(self, num_code: str):
        if isinstance(num_code, str) and len(num_code) >= 0:
            ...
            raise ValueError("num_code must be an string")

    @char_code.setter
    def char_code(self, char_code: str):
        parser = r"[a-zA-Z]+"
        if isinstance(char_code, str) and re.fullmatch(parser, char_code) is not None and len(char_code) == 3:
            self.__char_code = char_code.upper()
        else:
            raise ValueError("char_code must be a string of 3 letters")
    ...
    def name(self, name: str):
        if isinstance(name, str) and (len(name) > 0) and (len(name) <= 50):
            ...
            raise ValueError("name must be a string length not of None and at most 50 characters long")
    ...
    def nominal(self, nominal: int):
        if isinstance(nominal, int) and nominal >= 0:
            ...
            raise ValueError("nominal must be an integer greater than or equal to zero")
