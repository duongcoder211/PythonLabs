...
# file_logger = logging.getLogger("currency_file.log")
def hello():
    print("hello")
...
def get_currencies(currency_codes: list, url:str = "https://www.cbr.ru/scripts/XML_daily.asp?date_req=09.10.2025") -> dict:
    """
        ...
        if "ValCurs" in data:
            if data["ValCurs"]["Valute"]:
                ...
                for code in currency_codes:
                    if code in new_data_dict: currencies[code] = float(new_data_dict[code]["Value"].replace(",", "."))
                    else: currencies[code] = f"Код валюты '{code}' не найден."
        # print(currencies)
        return currencies
    ...
    #     # handle.write(f"Курс валюты имеет неверный тип: {te}\n")
    #     raise ValueError(f"Список курса валюты пустой: {ve}\n")
