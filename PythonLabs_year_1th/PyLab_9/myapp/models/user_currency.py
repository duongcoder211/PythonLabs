...
class UserCurrency:
    ...
    def id(self, id_: int):
        if isinstance(id_,int) and id_ >=0:
            ...
            raise ValueError("id must be an integer greater than or equal to 0")
    @user_id.setter
    def user_id(self, user_id: int):
        if isinstance(user_id, int) and user_id >= 0:
            self.__user_id = user_id
        else:
            raise ValueError("user id must be an integer greater than or equal to 0")
    @currency_id.setter
    def currency_id(self, currency_id):
        if isinstance(currency_id, str) and currency_id != "" and re.match(r'[a-zA-Z]', currency_id[0]) is not None:
            self.__currency_id = currency_id
        else:
            raise ValueError("currency id must be a nonempty string start with a letter")